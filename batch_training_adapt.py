...
class StreamAC(nn.Module):
    def __init__(self, n_obs=11, n_actions=3, hidden_size=128, lr=1.0, gamma=0.99, gae_lambda=0.95, kappa_policy=3.0, kappa_value=2.0, cbp=False, layernorm=False, optimizer="AdaptiveObGD",
        ...
        if self.optimizer_name == "AdaptiveObGD":
        ...
        elif self.optimizer_name == "FastTrac":
    ...
    def get_action_and_value(self, x, action=None):
        # Check if the input is a batch or a single observation
        is_batch = len(x.shape) > 1
        
        # Convert to tensor, adding a batch dimension if it's a single observation
        ...
        # sum(1) sums across the action dimension, which is correct for batches
        log_prob = probs.log_prob(action_tensor).sum(1)
        entropy = probs.entropy().sum(1)
        value = self.critic(x_tensor)

        # Return numpy array for action during rollout, and tensors for everything else
        # Squeeze the batch dimension out if the input was a single observation
        if not is_batch:
            return action_tensor.squeeze(0).numpy(), log_prob, entropy, value
        else:
            # During update, return the action tensor as well
    ...
    def update_params(self, obs_buf, act_buf, logp_buf, rew_buf, done_buf, val_buf, next_obs, next_done):
        # Convert lists to tensors and ensure they're 1D
        ...
        done_buf = torch.tensor(np.array(done_buf), dtype=torch.float32).flatten()
        ...
        # GAE Calculation
        with torch.no_grad():
            next_value = self.get_value(next_obs).flatten().item()  # Convert to scalar
        ...
        # All tensors should now be properly shaped
        b_obs = obs_buf
        b_logprobs = logp_buf
        b_actions = act_buf
        ...
        for epoch in range(self.update_epochs):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                if self.optimizer_name == "Adam" or self.optimizer_name == "FastTrac":
                    self.optimizer.zero_grad()
                    loss.backward()
                    nn.utils.clip_grad_norm_(self.parameters(), self.max_grad_norm)
                    self.optimizer.step()
                else: # ObGD / AdaptiveObGD
...
class StreamACRunner:
    ...
    def create_logs(self):
        ...
        if self.wandb_log:
            ...
            del config['agent'], config['env'], config['log_file'], config['eval_log_file']
    ...
    def evaluate(self):
        ...
        while episode_count < self.eval_episodes:
            ...
            if terminated or truncated:
                ...
                is_success = info.get('success', False) # Use .get for safety
    ...
    def train(self):
        ...
        for global_step in range(0, self.total_steps, self.num_steps):
                ...
                if done:
                    ...
                    if self.debug:
                        with open(self.log_file, 'a') as f:
                            f.write(f"Episodic Return: {episode_return}, Step {current_step}\n")
                    self.returns.append(episode_return)
                    self.term_time_steps.append(current_step)
            ...
            # PPO Update
            self.agent.update_params(obs_buf, act_buf, logp_buf, rew_buf, done_buf, val_buf, s, done)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_frequency', type=int, default=10_000, help="Evaluate every N steps")
    ...
    parser.add_argument('--lr', type=float, default=3e-8, help="Learning rate for Adam, or base LR for ObGD variants")
