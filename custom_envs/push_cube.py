...
class PushCubeEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    def _load_scene(self, options: dict):
        ...
        self.table = self.table_scene.table #save table for later access
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            target_region_xyz = xyz + torch.tensor([0.1 + self.x_offset + self.goal_radius, 0 + self.y_offset, 0])
    ...
    def get_cube_friction(self):
        # returns friction of cube as (static_friction, dynamic_friction)
        try:
            entity = self.obj._objs[0]
            for comp in entity.get_components():
                if isinstance(comp, sapien.physx.PhysxRigidDynamicComponent):
                    ...
                    if len(shapes) > 0:
                        mat = shapes[0].physical_material
    ...
    def set_cube_friction(self, static: float, dynamic: float):
        try:
            for entity in self.obj._objs:
                for comp in entity.get_components():
                    if isinstance(comp, sapien.physx.PhysxRigidDynamicComponent):
                        for shape in comp.get_collision_shapes():
                            mat = shape.physical_material
                            mat.static_friction = static
                            mat.dynamic_friction = dynamic
                            shape.physical_material = mat
            if hasattr(self.obj, "wake_up"):
                self.obj.wake_up()
                
        except Exception as e:
            print(f"DEBUG: Failed to set friction: {e}")

    def set_table_friction(self, static: float, dynamic: float):
        try:
            for entity in self.table._objs:
                for comp in entity.get_components():
                    if isinstance(comp, (sapien.physx.PhysxRigidStaticComponent, sapien.physx.PhysxRigidDynamicComponent)):
                        for shape in comp.get_collision_shapes():
                            mat = shape.physical_material
                            mat.static_friction = static
                            mat.dynamic_friction = dynamic
            ...
            print(f"DEBUG: Failed to set table friction: {e}")

    def get_table_friction(self):
        try:
            if not hasattr(self, "table"): return -1.0, -1.0
            
            entity = self.table._objs[0]
            for comp in entity.get_components():
                if isinstance(comp, (sapien.physx.PhysxRigidStaticComponent, sapien.physx.PhysxRigidDynamicComponent)):
                    shapes = comp.get_collision_shapes()
                    if shapes:
    ...
    def get_goal_position(self):
        try:
            return self.goal_region.pose.p.clone()
        except Exception:
            return None

    def set_goal_position(self, new_pos: torch.Tensor):
        try:
            current_q = self.goal_region.pose.q  # keep same rotation
            
            # for if unbatched input
            if new_pos.ndim == 1:
                new_pos = new_pos.unsqueeze(0)

            new_pose = Pose.create_from_pq(p=new_pos, q=current_q)
            self.goal_region.set_pose(new_pose)

        except Exception as e:
            print(f"DEBUG: Failed to set goal position: {e}")


    def set_goal_offset(self, offset: torch.Tensor):
